...
config.commandMap = {
  design: {
    ...
    enableContext: "AllowInsertChart && AllowEditObject",
    ...
    execute: async function (context, propertyName, fontItalicChecked) {
      ...
      if (!dataSetMap[tableData.bindingPath]) {
        ...
        tableSchema.tableId = tableData.bindingPath;
  ...
  updataData: {
    ...
    execute: async function (context, propertyName, fontItalicChecked) {
      ...
      tableSchema.dashboard.forEach((dashboard) => {
        dashboard.viewer.__reload();
      });
    },
  },
};
