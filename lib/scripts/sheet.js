...
async function showChartDesigner(spread, tableSchema, visualType) {
  ...
  dialog.querySelector(".close-btn").onclick = async function () {
    ...
    let chartName = "c" + timestamp + random;
    await designer.save({
      name: chartName,
      shouldPublish: true,
    });
    ...
    let floatingObjectName = "f" + timestamp + random;
    let customFloatingObject =
      new GC.Spread.Sheets.FloatingObjects.FloatingObject(floatingObjectName);
    ...
    setTimeout(() => {
      let viewer = createViewer(
        defaults,
        tableSchema.dashboard[tableSchema.dashboard.length - 1],
        document.getElementById(`chartDom${random}`)
      );
      tableSchema.dashboard[
        tableSchema.dashboard.length - 1
      ].floatingObjectName = floatingObjectName;
      tableSchema.dashboard[tableSchema.dashboard.length - 1].viewer = viewer;
