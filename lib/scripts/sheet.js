  ...
  design: {
    ...
    execute: async function (context, propertyName, fontItalicChecked) {
      ...
      if (!dataSetMap[tableData.bindingPath]) {
        ...
        dataSetMap[tableData.bindingPath] = tableSchema.dataset;
...
async function showChartDesigner(spread, schema, visualType) {
  ...
  dialog.querySelector(".close-btn").onclick = async function () {
    ...
    setTimeout(() => {
      ...
      // dataSetMap[tableSchema.bindingPath].dashboard.push({
      //   dashboardName: schema.dashboard[0].dashboardName,
      //   dashboardType: visualType,
      //   dashboardId: schema.dashboard[0].dashboardId,
      // });
      // console.log(dataSetMap);
    }, 0);
